document.addEventListener('DOMContentLoaded', () => {
    ...
    // This calculates a date 7 days from today to create urgency
    const setExpiryDate = () => {
        const today = new Date();
        const expiry = new Date(today.getTime() + 7 * 24 * 60 * 60 * 1000);
        
        // Format: "February 22, 2026"
        const options = { year: 'numeric', month: 'long', day: 'numeric' };
        const formattedDate = expiry.toLocaleDateString(undefined, options);
        
        // Set the hidden input value so it gets sent in the email
    ...
    emailForm.addEventListener('submit', function(event) {
        ...
        emailjs.sendForm(serviceID, templateID, this)
            .then(() => {
                ...
                emailForm.reset();
                setExpiryDate(); // Reset the date for the next user
            }, (err) => {
                submitBtn.innerText = 'Claim My 10% Off';
                submitBtn.disabled = false;
...
// --- 4. MENU TAB LOGIC ---
// This needs to be outside the DOMContentLoaded to be reachable by HTML onclick
